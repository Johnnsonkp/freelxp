...
import ColorThiefColor from "colorthief";
import type { Post } from "../../types/index.d.ts";
...
export default function PostDetail({ post }: { post: Post }) {
  ...
  const { palette }: ColorThiefColor = useColorThief(selectedPost?.metadata?.cover || '', { format: "hex", colorCount: 6 });

  useEffect(() => {
    if (palette && palette.length >= 6) {
      if (palette[5]) {
        const headerContent: HTMLElement | null = document.querySelector("#headerContent");
      ...
      if (paletteChildren) {
        Array.from(paletteChildren).forEach((child: HTMLElement, index: number) => {
          if (palette[index]) {
